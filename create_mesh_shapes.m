function varargout = create_mesh_shapes(varargin)
% CREATE_MESH_SHAPES MATLAB code for create_mesh_shapes.fig
%      CREATE_MESH_SHAPES, by itself, creates a new CREATE_MESH_SHAPES or raises the existing
%      singleton*.
%
%      H = CREATE_MESH_SHAPES returns the handle to a new CREATE_MESH_SHAPES or the handle to
%      the existing singleton*.
%
%      CREATE_MESH_SHAPES('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CREATE_MESH_SHAPES.M with the given input arguments.
%
%      CREATE_MESH_SHAPES('Property','Value',...) creates a new CREATE_MESH_SHAPES or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before create_mesh_shapes_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to create_mesh_shapes_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @create_mesh_shapes_OpeningFcn, ...
                   'gui_OutputFcn',  @create_mesh_shapes_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before create_mesh_shapes is made visible.
function create_mesh_shapes_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to create_mesh_shapes (see VARARGIN)

% Choose default command line output for create_mesh_shapes
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes create_mesh_shapes wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = create_mesh_shapes_OutputFcn(hObject, eventdata, handles) 
